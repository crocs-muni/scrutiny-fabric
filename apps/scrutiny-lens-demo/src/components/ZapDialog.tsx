...
export function ZapDialog({ target, children, className }: ZapDialogProps) {
  ...
  // Zaps feature temporarily removed (see repo Known Issues).
  // Keep the dialog UI but disable actual zap creation/payment.
  ...
  const openInWallet = () => {
      const handleZap = () => {
        toast({
          title: 'Zaps temporarily disabled',
          description: 'The zap feature is currently disabled in this public monorepo version.',
        });
        setIsZapping(false);
        setInvoice(null);
      };
